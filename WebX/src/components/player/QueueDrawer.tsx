...
export const QueueDrawer: React.FC = () => {
  ...
  return (
    <>
      ...
      <aside
        className="sheet-right fixed top-0 right-0 bottom-[calc(var(--webx-player-height)+env(safe-area-inset-bottom,0px))] md:bottom-[var(--webx-player-height)] z-40 w-full sm:w-[400px] elev-1 border-l border-outline-variant/60 flex flex-col shadow-md3-3 md:shadow-none"
        data-open={open}
        aria-hidden={!open}
        // @ts-expect-error React 19 supports the inert attribute
        inert={open ? undefined : ''}
      >
        <div className="flex items-center justify-between pl-5 pr-3 h-[var(--webx-topbar-height)] shrink-0 border-b border-outline-variant/60">
